...
The raw `card_art_url` string also appears in an unrelated web-redirect intent path in this build, so it is not used as a guessed hook point. The tap-confirmation target is limited to the package-manager-declared `TapActivity` with action `com.google.android.gms.tapandpay.tap.TAP_EVENT`; it scans the visible card-art View and labels without parsing the activity's Intent payload. Suica/QUICPay can match their unique stored card-type fingerprint, or use the existing 15-second Wallet selection link to disambiguate a deliberate recent selection only when its network matches the visible payment label. The module does not hook TapAndPay methods or touch NFC/HCE, credentials, transaction processing, attestation, or integrity checks. Actual tap-to-pay was not exercised for validation.
